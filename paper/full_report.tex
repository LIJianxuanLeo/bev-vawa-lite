...
episodes on the Gibson val split \emph{without} the reactive safety
wrapper, the trained policy's SR is $0.00$–$0.01$, statistically
indistinguishable from a perception-free \texttt{straight} baseline
($0.02$–$0.07$). Mean CollisionRate is $1.0$: every episode terminates
on collision before reaching the goal. As discussed in \S9.3, this is
consistent with the covariance-shift failure mode of pure
imitation — our training data is generated via teleport-based expert
trajectories, which by construction never visit the adversarial states
the closed-loop policy encounters.}

\zh{\textsf{\textbf{闭环差距。}在 Gibson val 上跑四种子 $\times$ 100 episodes
的闭环评估\emph{不挂载}反应式安全模块时，策略 SR 为 $0.00$–$0.01$，与
无感知的 \texttt{直走} 基线（$0.02$–$0.07$）在统计上不可区分；平均碰撞
率 $1.0$，即每条 episode 在触达目标前都因碰撞终止。如 \S9.3 所述，这与
纯模仿的协方差偏移失败模式完全一致——我们的训练数据由 teleport 式专家
轨迹生成，构造上从未访问闭环策略会遇到的敌对状态。}}

\en{\textbf{What is needed to close the gap} (in order of engineering
difficulty):
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Gibson-tuned reactive safety.} The PIB-Nav safety
        parameters ($\mathtt{warn\_m}, \mathtt{arc\_frac}, \ldots$)
        were tuned for $5$–$7\,$m procedural rooms; Gibson scenes have
        narrower corridors and lower ceilings, which likely requires
        a per-track parameter sweep. Expected cost: one additional
        $4$-seed eval pass, $\sim$7 min and $<$¥1 GPU billing.
  \item \textbf{Closed-loop data aggregation (DAGger).}~Run the
        partially-trained policy against the Habitat simulator, record
        the states it visits, and query the pathfinder for the correct
        waypoint at each of them. Fold the augmented trajectories back
        into Stage-B/C shards. This is the canonical fix for covariance
        shift and is expected to bring the Gibson track to the same
        regime as PIB-Nav + safety.
  \item \textbf{PPO fine-tune (Stage D).}~Add a residual PPO
        optimisation layer over the Stage-C checkpoint using the Habitat
        reward (geodesic progress minus collision penalty). This is
        the heaviest option and is only justified if DAGger alone is
        insufficient.
\end{enumerate}}

\zh{\textsf{\textbf{闭合差距所需的工作}（按工程难度升序）：
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Gibson 专属反应式安全调参。}PIB-Nav 安全层超参
        ($\mathtt{warn\_m}, \mathtt{arc\_frac}, \ldots$) 是针对
        $5$–$7\,$m 程序化房间调的；Gibson 场景的走廊更窄、天花板更
        低，需要一次 per-track 扫参。预期成本：多一次四种子评估，
        $\sim 7$ 分钟，GPU 计费 $<$¥1。
  \item \textbf{闭环数据回填（DAGger）。}用半训练策略跑 Habitat，记
        录其访问状态并查询 pathfinder 得到正确 waypoint，将增广轨迹
        回填到 Stage-B/C 分片。这是协方差偏移的标准修复，预期能让
        Gibson 线达到与 PIB-Nav + safety 同档表现。
  \item \textbf{PPO 微调（Stage D）。}在 Stage-C 检查点之上叠加残差
        PPO，奖励为 Habitat 的测地进度减碰撞惩罚。此方案开销最大，
        仅当 DAGger 单独不足时才值得投入。
\end{enumerate}}}
