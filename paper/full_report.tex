...
% =====================================================================
\begin{abstract}\noindent
\textbf{EN.} We propose a \textbf{BEV-native dual-system architecture}
for depth-only PointGoal navigation in which two cooperating policy
heads operate on a \emph{single shared geometric bird's-eye-view (BEV)
state}: a \textbf{Vision-Action (VA) head} provides fast reflexive
waypoint selection from $K{=}5$ anchor candidates ($\sim\!0.3$\,ms
latency), and a \textbf{World-Action (WA) head} provides slow
deliberative evaluation via an $H{=}3$-step latent rollout with
per-step supervision on collision risk, goal progress,
ensemble-variance uncertainty, and pathfinder-computed reachability
($\sim\!3$\,ms latency). An \textbf{analytic fusion rule}
...
delta is $4\times$ smaller than single-seed noise). A four-intervention
study on the 3D-scanned Gibson distribution validates the
architecture's \textbf{scope}: imitation-only dual systems cannot bridge
geometric covariance shift without closed-loop correction (DAGger) or
reinforcement learning — a design boundary rather than a failure mode.

\vspace{0.6em}
\noindent\textbf{中文摘要。}\color{zhcol}
\textsf{本文提出一个用于纯深度 PointGoal 导航的\textbf{ BEV-原生 双系统架构}：
两个策略头在\textbf{共享几何鸟瞰图（BEV）状态}上协同工作 ——
...
标准差比单种子噪声小 $4\times$）。在 3D 扫描的 Gibson 分布上，四项干预
实验\textbf{量化了}架构的\textbf{适用边界}：纯模仿式双系统无法仅靠参数
调优或单轮闭环回填跨越几何协方差偏移，这是一项\emph{设计边界声明}而非
失败论断。}\color{black}
\end{abstract}
...
\item \textbf{Geometric BEV as the right shared representation.} We
show that a multi-channel occupancy / free-space / goal-prior BEV
produced by a differentiable pinhole depth-lift strictly outperforms a
pooled-feature "flat" encoder of matched parameter count, supporting
the claim that navigation decisions need metric geometric state, not
pixel features (Table~\ref{tab:bev-ablation}).
...
项都有对应实验量化：
\begin{enumerate}[leftmargin=*, itemsep=3pt]
\item \textbf{几何 BEV 是正确的共享表示。}通过可微针孔深度投影得到的
"占用 / 自由空间 / 目标先验" 多通道 BEV，在同参数量对照下严格优于基于
特征池化的"平展"编码器；这支持我们的论点：导航决策需要\emph{度量式几
何状态}，而非像素特征（见表~\ref{tab:bev-ablation}）。
