...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Gibson-tuned reactive safety.} Confirmed empirically in
        Table~\ref{tab:gibson-eval}: PIB-Nav's safety knobs saturate in
        Gibson corridors and yield a null paired delta
        ($+0.008 \pm 0.009$ SR). The minimum viable fix is a parameter
        sweep over $\mathtt{warn\_m}, \mathtt{near\_m},
        \mathtt{side\_warn\_m}$ re-centred on Gibson corridor widths
        ($\sim 1.5\,$m), which can be done as a grid of
        $4$-seed evals without any retraining. Expected cost: $\sim$4
        parameter points $\times\,$7 min each $\approx 30$ min, $<$¥1
        GPU billing.
